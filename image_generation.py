...
def _():
    ...
    return animation, mo, plt, random, to_pil_image, torch
...
def _(categories, mo):
    ...
    generate_btn = mo.ui.run_button(label="Generate new image")

    mo.vstack([dropdown, generate_btn])
    return (generate_btn,)
...
@app.cell
def _(generate_btn, mo, random, to_pil_image, torch):
    def generate_noise(
        batch_size: int = 1,
        channels: int = 3,
        height: int = 256,
...
def _(animation, mo, plt, tensor):
    def generate_animation():
        ...
        init_patch_size = 1.0
        grid_start_x = center_x - (num_cols * init_patch_size) / 2.0
        grid_start_y = 4.8 
        split_gap_max = 0.6

        seq_patch_size = 0.9
        seq_spacing = 1.15
        seq_total_width = (total_patches - 1) * seq_spacing + seq_patch_size
        seq_start_x = center_x - (seq_total_width / 2.0)
        ...
        p = img.shape[0] // num_rows

        patches = [img[r*p:(r+1)*p, c*p:(c+1)*p, :] for r in range(num_rows) for c in range(num_cols)]

        im_objs = []
        for idx, p_data in enumerate(patches):
            r, c = divmod(idx, num_cols)
            x0 = grid_start_x + c * init_patch_size
            y0 = grid_start_y - r * init_patch_size
            im = ax.imshow(p_data, extent=[x0, x0 + init_patch_size, y0, y0 + init_patch_size], origin='upper')
            im_objs.append(im)

        title = ax.text(center_x, title_y, "Input Image", ha='center', va='center', 
                        fontsize=22, fontweight='bold', color='white')

        embed_texts = []

        def update(frame):
            if frame < frame_pause:
                title.set_text("Input Image")

            elif frame < frame_split:
                title.set_text("Splitting into Patches")
                progress = (frame - frame_pause) / float(frame_split - frame_pause - 1)
                gap = progress * split_gap_max
                for idx, im in enumerate(im_objs):
                    r, c = divmod(idx, num_cols)
                    c_offset = c - (num_cols - 1) / 2.0
                    r_offset = (num_rows - 1) / 2.0 - r 

                    x0 = grid_start_x + c * init_patch_size + c_offset * gap
                    y0 = grid_start_y - r * init_patch_size + r_offset * gap
                    im.set_extent([x0, x0 + init_patch_size, y0, y0 + init_patch_size])

            elif frame < frame_flatten:
                title.set_text("Flattening into 1D Sequence")
                progress = (frame - frame_split) / float(frame_flatten - frame_split - 1)
                for idx, im in enumerate(im_objs):
                    r, c = divmod(idx, num_cols)
                    ...
                    start_x0 = grid_start_x + c * init_patch_size + c_offset * split_gap_max
                    start_y0 = grid_start_y - r * init_patch_size + r_offset * split_gap_max

                    target_x0 = seq_start_x + idx * seq_spacing
                    target_y0 = seq_y

                    curr_x0 = start_x0 + (target_x0 - start_x0) * progress
                    curr_y0 = start_y0 + (target_y0 - start_y0) * progress
                    curr_w = init_patch_size + (seq_patch_size - init_patch_size) * progress

                    im.set_extent([curr_x0, curr_x0 + curr_w, curr_y0, curr_y0 + curr_w])

            elif frame < frame_embed:
                title.set_text("Transform the input to feed the model")
                if not embed_texts:
                    for idx in range(total_patches):
                        cx = seq_start_x + idx * seq_spacing + (seq_patch_size / 2.0)
                        t = ax.text(cx, pos_embed_y, "+ Pos\nEmbed", ha='center', va='center', 
                                    fontsize=9, color='white', fontweight='bold',
                                    bbox=dict(facecolor='#0000aa', alpha=0.9, boxstyle='round,pad=0.4', edgecolor='white'))
                        embed_texts.append(t)

                    t_global = ax.text(center_x, global_embed_y, "+ Timestep and class embeddings", 
                                       ha='center', va='center', fontsize=18, color='white', 
                                       bbox=dict(facecolor='#006600', alpha=0.9, boxstyle='round,pad=0.6', edgecolor='white'))
                    embed_texts.append(t_global)

            return im_objs + [title] + embed_texts

        ani = animation.FuncAnimation(fig, update, frames=total_frames, interval=60, blit=False)
        plt.close(fig)

        return mo.Html(ani.to_html5_video())

    return (generate_animation,)


@app.cell
def _(generate_animation, generate_btn, mo):
    if generate_btn.value:
        anim_output = generate_animation()
    else:
        anim_output = mo.md("*Generate an image to see the input conversion process.*")
